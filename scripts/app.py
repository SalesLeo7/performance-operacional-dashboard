...
with col_bot2:
    ...
    top_delay_clients = out_time_df.groupby('Client').size().sort_values(ascending=False).head(10).reset_index(
        name='Atrasos')
...
with st.expander("Visualizar Dados Brutos Detalhados"):
    st.dataframe(df_filtered[['Client', 'Pre-Advice ID', 'Creation Date', 'Finish Date', 'working_hours',
                              'performance']].sort_values(by='working_hours', ascending=False))
